...
/// struct for typed errors of method [`v2_pageinsights_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum V2PageinsightsGetError {
    UnknownValue(serde_json::Value),
}

/// struct for typed errors of method [`v2_projects_project_id_pageinsights_get`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum V2ProjectsProjectIdPageinsightsGetError {
    UnknownValue(serde_json::Value),
}
...
pub async fn v2_pageinsights_get(configuration: &configuration::Configuration, domain: &str, path: &str, date: Option<String>) -> Result<(), Error<V2PageinsightsGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v2/pageinsights", local_var_configuration.base_path);
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<V2PageinsightsGetError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}

pub async fn v2_projects_project_id_pageinsights_get(configuration: &configuration::Configuration, project_id: &str, domain: Option<&str>) -> Result<(), Error<V2ProjectsProjectIdPageinsightsGetError>> {
    let local_var_configuration = configuration;

    let local_var_client = &local_var_configuration.client;

    let local_var_uri_str = format!("{}/v2/projects/{projectId}/pageinsights", local_var_configuration.base_path, projectId=crate::apis::urlencode(project_id));
    let mut local_var_req_builder = local_var_client.request(reqwest::Method::GET, local_var_uri_str.as_str());
    ...
    if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
        ...
        let local_var_entity: Option<V2ProjectsProjectIdPageinsightsGetError> = serde_json::from_str(&local_var_content).ok();
        let local_var_error = ResponseContent { status: local_var_status, content: local_var_content, entity: local_var_entity };
        Err(Error::ResponseError(local_var_error))
    }
}
